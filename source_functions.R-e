...
source_functions <- function(get_best_model_versions = FALSE, get_cause_metadata = FALSE, get_covariate_estimates = TRUE,
                             ...
                             save_results_epi = FALSE, get_pct_change = FALSE, split_epi_model = FALSE,
                             all = FALSE) {

  # set up OS flexibility
  setup()
  base <- paste0(j_root, "REDACTED/current/r/")
  ...
  if (all == TRUE) {
    ...
    mapply(source, unname(filepaths))
...
order(names(unlist(as.list(args(source_functions))))) == ls()
